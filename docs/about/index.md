...
* Ad hoc (called arbitrarily from within other service actions)
  * `depends_on` (usage: `depends_on { Nested::Action.call }`) ensures any failure from a nested service is bubbled up to the top level (by default, as if the failure had happened there directly).
