...
module Action
  module Hoist
    ...
    module InstanceMethods
      ...
      def hoist(error_prefix: nil)
        raise ArgumentError, "#hoist must be given a block to execute" unless block_given?

        result = begin
          yield
        rescue StandardError => e
          warn "hoisted block raised an exception: #{e.message}"
          MinimalFailedResult.new(error: self.class.determine_error_message_for(e))
        end

        # This ensures the last line of hoist was an Action call (CAUTION: if there are multiple
        # calls per block, only the last one will be checked!)
        unless result.respond_to?(:ok?)
          raise ArgumentError,
                "#hoist is expected to wrap an Action call, but it returned a #{result.class.name} instead"
        end
